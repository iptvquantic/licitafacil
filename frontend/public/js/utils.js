// public/js/utils.js
// Utilitários globais do LicitaFácil — sem template literals aninhados
...
function api(method, path, body, isFormData) {
  ...
  return fetch(API_URL + path, opts).then(function(r) {
    if (r.status === 401) {
      // Redirecionar para login apenas se não estiver nas páginas públicas
...
function apiFormPut(path, formData) { return api('PUT', path, formData, true); }
...
function setLoading(btn, loading, texto) {
  ...
  if (loading) {
    ...
    btn.disabled = true;
    btn.innerHTML = '<span class="spinner"></span>' + (texto ? esc(texto) : 'Aguarde...');
  } else {
    btn.disabled = false;
    btn.innerHTML = btn.dataset.originalText || texto || 'Enviar';
  }
}
